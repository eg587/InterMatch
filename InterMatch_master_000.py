...
def calculate_elastic_strain_supercell(mpid1, mpid2, V1_v, V2_v, V1_u, V2_u):
    ...
    if data_mpid_1["elasticity"] is not None and data_mpid_2["elasticity"] is not None:
        ...
        a_eff = (
            (axial_comp_max + axial_comp_min)
            * a_max
            * a_min
            / (axial_comp_max * a_max + axial_comp_min * a_min)
        )
        b_eff = (
            (axial_comp_max + axial_comp_min)
            * b_max
            * b_min
            / (axial_comp_max * b_max + axial_comp_min * b_min)
        )
...
def compute_best_supercell(
    ...
    rotmat = np.array(
        [
            [cos(np.deg2rad(theta)), -sin(np.deg2rad(theta))],
            [sin(np.deg2rad(theta)), cos(np.deg2rad(theta))],
        ]
    )
    ...
    for x in range(len(bcells)):
        ...
        area_ratio_v = round(
            abs((V1_v[0] * V2_v[1] - V1_v[1] * V2_v[0]) / (br1x * br2y - br1y * br2x))
        )
        area_ratio_u = round(
            abs((V1_u[0] * V2_u[1] - V1_u[1] * V2_u[0]) / (a1x * a2y - a1y * a2x))
        )
        ...
        if (
            abs(strain[0]) < strain_max
            and abs(strain[1]) < strain_max
            and abs(strain[2]) < strain_max
            and atoms_max > total_atoms > atoms_min
            and round(bcells[x][0][0][0]) == round(bcells[x][0][1][0])
            and round(bcells[x][0][0][1]) == round(bcells[x][0][1][1])
            and round(bcells[x][1][0][0]) == round(bcells[x][1][1][0])
            and round(bcells[x][1][0][1]) == round(bcells[x][1][1][1])
        ):
...
materials_of_interest = mpr.query(
    criteria={
        "elements": {"$in": target_elements, "$all": ["S"]},
        "nelements": 2,
        "efermi": {"$exists": True, "$ne": None},
    },
    properties=["material_id", "pretty_formula"],
)
mpids_list = [
    x["material_id"] for x in materials_of_interest if x["material_id"] is not None
]
stable_mpids_list = []
...
# compute pairs from queried list:
# entries = []
